class UsersController < ApplicationController
  ...
  def create
    ...
    if @user.save
      ...
      if @user.confirm_role.name == "Instructor" or @user.confirm_role.name == "Administrator"
  ...
  def keys
    if (params[:id].nil?) || ((current_user_role? == "Student") &&  (session[:user].id != params[:id].to_i))
      ...
      @user = User.find(params[:id])
      @participant=AssignmentParticipant.find_by_user_id(params[:id])
      if @participant != nil
        @private_key = @participant.generate_keys(@user)
      end
    end
  end
