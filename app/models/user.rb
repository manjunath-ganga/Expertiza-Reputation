class User < ActiveRecord::Base
  ...
  def get_available_users(name)    
    lesser_roles = confirm_role.get_parents
    all_users = User.find(:all, :conditions => ['name LIKE ?', "#{name}%"], :limit => 20) # higher limit, since we're filtering
    visible_users = all_users.select{|user| lesser_roles.include? user.confirm_role}
 ...
 # end
  ...
  def self.return_checkbox_value(checkbox_value)
    if checkbox_value==true
      "yes"
    elsif checkbox_value ==false
      "no"
    else
      ""
    end
  ...
  def self.export(csv, parent_id, options)
    ...
    users.each {|user|
      ...
      if (options["personal_details"] == "true")
        temp_csv.push(user.name, user.fullname, user.email)
      end
      if (options["role"] == "true")
        temp_csv.push(user.confirm_role.name)
      end
      if (options["parent"] == "true")
        temp_csv.push(user.parent.name)
      end
      if (options["email_options"] == "true")
        temp_csv.push(user.email_on_submission, user.email_on_review, user.email_on_review_of_review)
      end
      if (options["handle"] == "true")
        temp_csv.push(user.handle)
      end
      csv << temp_csv
    }
  end
  ...
  def is_teaching_assistant_for?(student)
    return false if self.confirm_role.name != 'Teaching Assistant'
    return false if student.confirm_role.name != 'Student'
