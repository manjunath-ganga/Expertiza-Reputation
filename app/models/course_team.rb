class CourseTeam < Team
  ...
  def self.import_teams(row,session,id,options)
    if row.length < 2
    ...
    if currTeam != nil
        options_output = currTeam.check_options(options)
        if options_output == 0
          return
        elsif options_output == 1
          currTeam=nil
        elsif options_output == 2
          name=generate_team_name()
          currTeam=nil
        end
    end
  ...
  def check_options(options)
    if options[:handle_dups] == "ignore"
      return 0
    elsif options[:handle_dups] == "replace"
      self.delete      #added this function
      return 1
    elsif options[:handle_dups] == "rename"
      return 2
    end
  end
